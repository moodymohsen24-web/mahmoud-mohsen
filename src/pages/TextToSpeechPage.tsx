...
import { ttsStorage } from '../utils/ttsStorage';
import { splitText } from '../utils/textChunking';
...
const TextToSpeechPage: React.FC = () => {
    ...
    const loadData = async () => {
        if(user) {
            ...
            try {
                ...
                const sanitizedKeys = [...new Set(rawKeys.map((k: any) => (typeof k === 'object' && k !== null && 'key' in k ? k.key : k)).filter((k: any): k is string => typeof k === 'string' && k.trim().length > 0).map(k => k.trim()))] as string[];
                ...
                if (savedStateJSON) {
                    ...
                    if (chunksFromStorage.length > 0) {
                        const updatedChunks = await Promise.all(chunksFromStorage.map(async (chunk): Promise<ConvertedChunk> => {
                            if (chunk.status === 'success' && user?.id) {
                                const blob = await ttsStorage.getChunk(user.id, chunk.id);
                                if (blob) {
                        ...
                        setConvertedChunks(updatedChunks);
                    } else {
                        setConvertedChunks([]);
                    }
                }
                
                // Silent balance check on load
                if (sanitizedKeys.length > 0) {
                    checkBalances(sanitizedKeys, true);
                }
            } catch (e) { 
                console.error("Failed to load TTS data", e); 
            } finally {
                setIsLoading(false);
            }
        }
  ...
  const handleClearSession = useCallback(async () => {
    ...
    if (user) {
        await ttsStorage.clearUserChunks(user.id);
    }
    setFullText('');
    setLogMessages([]);
    setConvertedChunks([]);
  ...
  useEffect(() => {
    ...
    if (textToLoad && user) {
      localStorage.removeItem(LOCAL_STORAGE_KEY);
      ttsStorage.clearUserChunks(user.id);
      setLogMessages([]);
      setConvertedChunks([]);
      setProgress(0);
      setCurrentProcess(0);
  ...
  useEffect(() => {
    ...
    const newChunks = splitText(fullText, min, max);
  ...
  const startConversion = async () => {
    ...
    for (let i = 0; i < initialChunks.length; i++) {
        ...
        if (result.success && result.audioUrl && result.audioBlob) { 
            ...
            if (user) {
                await ttsStorage.saveChunk(user.id, chunk.id, result.audioBlob);
            }
  ...
  const handleRetryChunk = async (chunkId: number) => {
    ...
    if (result.success && result.audioUrl && result.audioBlob) {
        ...
        await ttsStorage.saveChunk(user.id, chunkId, result.audioBlob);
